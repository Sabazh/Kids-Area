...
import { lightOrange } from '@/themes/colors'
...
const spacePage = () => {
  const repeatedData = Array.from({ length: 144 }, (_, index) => ({
    ...spaceData[index % spaceData.length],
    id: index + 1,
  }))
  ...
  return (
    <div>
      <ImgSlider />
      <div className="cont">
        <div className="mt-5-0 h-8-0 rounded-1-0 cursor-pointer bg-lightOrange">
          <h2 className="text-center sm:flex font-fR text-white text-1-8 sm:text-2-5 sm:ml-9-4 py-2-0 uppercase">
            ძალიან ბევრ დაბადების დღის ცენტრს ნახავ აქ
          </h2>
        </div>
        ...
        <div className="my-5-0 sm:my-10-0">
          <Pagination
            ...
            background="bg-lightOrange px-1-8 py-0-8 rounded-1-0"
          />
        </div>
      </div>
      <Footer backgroundColor={lightOrange} />
    </div>
  )
}
